...
export class login_request {
    public static UserInfo( data:{ UserID:number, Name:string, Accounts:string, Password:string, FaceID:number, Gender:number, Age:number, Level:number, Gold:number, PassPortID:string, RealName:string, AgentID:number, SpreaderGameID:number, ClientAddr:number, MachineCode:string } ) { login_packet_define[0].sendToChannel(ChannelDefine.game, data, false); }
