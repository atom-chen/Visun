...
export default class LoginSihaiMgr extends ModelBase {
	private static _instance:LoginSihaiMgr = null;
	private _hall_ws_list = null;

	private constructor(){
		super();
	}
    public static getInstance() : LoginSihaiMgr {
        if(!LoginSihaiMgr._instance) { LoginSihaiMgr._instance = new LoginSihaiMgr; }
        return LoginSihaiMgr._instance;
	}
	public static delInstance() : void {
		if(LoginSihaiMgr._instance) {
			LoginSihaiMgr._instance.clear();
			LoginSihaiMgr._instance = null;
		}
	}
	public on_clear() {
	...
	private onNetFail(info) {
		if(info.cmd === login_msgs.UserLogInResp) {
			...
			//session过期，需重新登录
			if(info.errCode === 404) {
				LoginSihaiMgr.getInstance().loginAsYouke();
			}
		}
	}
	...
	private onNetWorkChange(net:any)
    {
        if(ChannelDefine.hall == net.getName())
        {
            ...
            {
				if(!LoginSihaiMgr.getInstance().checkLogin(false)) {
					cc.log("尚未登录");
