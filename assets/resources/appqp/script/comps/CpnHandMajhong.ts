...
import CpnMajhong from "./CpnMajhong";

const {ccclass, property} = cc._decorator;

@ccclass
export default class CpnHandMajhong extends cc.Component {
    @property(cc.Prefab)
    ...
    public resetCards(cards:Array<MajhongCode>|Uint8Array) : CpnHandMajhong {
        if(!isNil(cards) && cards.length == this.node.children.length) {
            var bSame = true;
            var childs = this.node.children;
            for(var n=0; n<cards.length; n++) {
                if(cards[n] != childs[n].getComponent(CpnMajhong).getCode()) {
                    bSame = false;
                    break;
                }
            }
    ...
    public delCards(cards:Array<MajhongCode>|Uint8Array) {
        if(cards===null || cards===undefined) {
            return;
        }
        for(var i in cards) {
    ...
    public addCards(cards:Array<MajhongCode>|Uint8Array) {
        if(cards===null||cards===undefined) {
            return;
        }
        for(var i in cards) {
    ...
    public playOpen(bAni:boolean = true) {
        ...
        for(var i=0, len=childs.length; i<len; i++) {
            var cpn = childs[i].getComponent(CpnMajhong);
            if(bAni){
                cpn.playFlip(i*0.1);
            } else {
    ...
    private addCard(cardV:MajhongCode) : CpnMajhong {
        var card = cc.instantiate(this.pokerTemplate);
        var comp:CpnMajhong = card.getComponent(CpnMajhong);
        comp.setCode(cardV);
    ...
    private delCard(cardV:MajhongCode) {
        var childs = this.node.children;
        for(var i in childs) {
            if(childs[i].getComponent(CpnMajhong).getCode()==cardV) {
                childs[i].destroy();
                break;
            }
        }
